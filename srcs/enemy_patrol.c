...
/*   Updated: 2023/12/24 00:15:24 by nazouz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int		i;
	long current_time = clock();  // Or use gettimeofday()

	i = 0;
	if (current_time - last_move_time >= 50000) // Check for 500 millisecond interval
	{
		...
		last_move_time = current_time;  // Update for next iteration
	}
	return (0);
}